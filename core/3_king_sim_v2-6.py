...
# =================================================================
# [3-Kingdom] 專案代號：三國演義 - 實戰模擬探測器 (v1.6 數據分析版)
# 模式：24/7 零足跡偵察 (Public API)
# 更新：增加完整成交係數紀錄 (價格、時間、路徑細節)，儲存於 result/*.csv 用於後續分析
# =================================================================
...
class ThreeKingdomsSim:
    def __init__(self, initial_capital=20000.0):
        ...
        logger.info(f"🚀 3-Kingdom v1.6 [數據分析版] 啟動 | 數據存儲: {self.csv_path}")
    ...
    def simulate_strike(self, path, account_name):
        """
        模擬執行並返回詳細係數
        返回: (roi, path_str, jitter, leg_prices, step_details)
        """
        ...
        return roi, " -> ".join(step_details), jitter, leg_prices, step_details
    ...
    def run(self):
        ...
        while True:
            ...
            for path in sample_paths:
                ...
                roi, path_str, jitter, leg_prices, steps = self.simulate_strike(path, account)

                if roi > best_roi_this_round:
                    best_roi_this_round = roi
                    best_path_this_round = path_str

                # 🛠️ 紀錄門檻放寬至 -0.5%，以儲存更多數據係數用於分析
                if roi > -0.5 and path_str is not None:
                    log_entry = {
                        ...
                        'p1': leg_prices[0] if len(leg_prices) > 0 else 0,
                        'p2': leg_prices[1] if len(leg_prices) > 1 else 0,
                        'p3': leg_prices[2] if len(leg_prices) > 2 else 0,
